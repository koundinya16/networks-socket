...
/* -------------------------------tcpserver.c------------------------------------
 Author : Koundiya 
 -----------------------------------------------------------------------------
 A UDP math server that returns the result of mathematical operation

 

 Message format : <num1> <operation> <num2>
 Available operations : add , sub , mul , div , exp
    ...
    // Listen for connections
     ...
     while(1)
     {
      ...
      if (num_data_recvd < 0) 
       ...
       printf("\n $$ : Received %d bytes of data : %s from ",num_data_recvd, receive_buff);
      ...
      sprintf(transmit_buff,"%f",result);


      // Transmit the message to the Client
       printf("Data echo : %s ,%d",receive_buff,num_data_recvd);
      num_data_sent = sendto(host_socket_fd,transmit_buff,strlen(transmit_buff),0,(struct sockaddr *)&client_sockaddr,client_sockaddr_len);
      if (num_data_sent < 0) 
